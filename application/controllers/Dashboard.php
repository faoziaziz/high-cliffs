...
class Dashboard extends CI_Controller {
	...
	function chart2(){
			...
			select   a.Nomor, (a.FileTime)as DateTime, a.DeviceId, c.Tenant, if(a.nilai like '%,%',replace(a.nilai,',',''),replace(a.nilai,'.',''))as dpp,if(a.pajak like '%,%',replace(a.pajak,',',''),replace(a.pajak,'.',''))as pajak, if(a.nilai like '%,%',replace(a.nilai,',',''),replace(a.nilai,'.',''))as total
			from Transaksi a join DeviceTable b on (a.DeviceId = b.DeviceId) join Tenant c on  (b.tenant = c.id)
			WHERE ((MONTH(a.FileTime)=MONTH(CURDATE())) AND (YEAR(a.FileTime)=YEAR(CURDATE()))) and a.DeviceId in ('APS010700020','APS010700030','APS010700040') group by a.Nomor,a.nilai
			union all
			select   a.Nomor, (a.FileTime)as DateTime, a.DeviceId, c.Tenant, if(a.nilai like '%,%',replace(a.nilai,',',''),replace(a.nilai,'.',''))as dpp,if(a.pajak like '%,%',replace(a.pajak,',',''),replace(a.pajak,'.',''))as pajak, if(a.nilai like '%,%',replace(a.nilai,',',''),replace(a.nilai,'.',''))as total
			from Transaksi a join DeviceTable b on (a.DeviceId = b.DeviceId) join Tenant c on  (b.tenant = c.id)
	...
	function chart(){
			...
			select   a.Nomor, (a.FileTime)as DateTime, a.DeviceId, c.Tenant, if(a.nilai like '%,%',replace(a.nilai,',',''),replace(a.nilai,'.',''))as dpp,if(a.pajak like '%,%',replace(a.pajak,',',''),replace(a.pajak,'.',''))as pajak, if(a.nilai like '%,%',replace(a.nilai,',',''),replace(a.nilai,'.',''))as total
			from Transaksi a join DeviceTable b on (a.DeviceId = b.DeviceId) join Tenant c on  (b.tenant = c.id)
			WHERE ((MONTH(a.FileTime)=MONTH(CURDATE())) AND (YEAR(a.FileTime)=YEAR(CURDATE()))) and a.DeviceId='APS010700020' group by a.Nomor,a.nilai
			union all
			select   a.Nomor, (a.FileTime)as DateTime, a.DeviceId, c.Tenant, if(a.nilai like '%,%',replace(a.nilai,',',''),replace(a.nilai,'.',''))as dpp,if(a.pajak like '%,%',replace(a.pajak,',',''),replace(a.pajak,'.',''))as pajak, if(a.nilai like '%,%',replace(a.nilai,',',''),replace(a.nilai,'.',''))as total
			from Transaksi a join DeviceTable b on (a.DeviceId = b.DeviceId) join Tenant c on  (b.tenant = c.id)
	    ...
	    //$data['map'] = $this->googlemaps->create_map();
			...
			function daily_revenue(){
					/*return $this->db->query("SELECT  sum(a.total1) as total
												from(
												SELECT nomor, date(filetime)as date, DeviceId , if(nilai like '%,%',replace(nilai,',',''),replace(nilai,'.',''))as total1
												FROM Transaksi
												WHERE nomor<>'' AND DATE(`filetime`) = CURDATE()  and DeviceId='APS010700020' group by Nomor,nilai
												union all
												SELECT nomor, date(filetime)as date, DeviceId , if(nilai like '%,%',replace(nilai,',',''),replace(nilai,'.',''))as total1
												FROM Transaksi
					...
					$query=	$this->db->query("SELECT  sum(a.total1) as total
												from(
												SELECT distinct nomor, date(filetime)as date, DeviceId , if(nilai like '%,%',replace(nilai,',',''),replace(nilai,'.',''))as total1
												FROM Transaksi
												WHERE nomor<>'' AND DATE(`filetime`) = CURDATE()
