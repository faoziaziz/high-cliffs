...
class Tenant extends CI_Controller {
    ...
    function data() {
        ...
        $columns = array(
			...
			array('db' => 'JenisPenjualan', 'dt' => 'JenisPenjualan'),
			array('db' => 'Additional', 'dt' => 'Additional'),
            array(
                'db' => 'id',
                'dt' => 'aksi',
                'formatter' => function($d) {
                    //return "<a href='edit.php?id=$d'>EDIT</a>";
                    return '<a class="btn btn-xs btn-primary tooltips" href="javascript:void(0)" title="Edit" onclick="edit_person('."'".$d."'".')">
						<i class="glyphicon glyphicon-pencil"></i></a> <a class="btn btn-xs btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_person('."'".$d."'".')">
    ...
    function add(){
		...
		$Tenant = preg_replace('/\s+/', '', $Tenant);
        $data = array(
            'DeviceId' => $Tenant.'-MANUAL',
            'Tenant' => $this -> input -> post('txtTenant', TRUE),
            ...
            'Kode' => $this -> input -> post('txtKode', TRUE),
            'JenisPenjualan' => $this -> input -> post('txtJenisPenjualan', TRUE),
            'Additional' => $this -> input -> post('txtAdditional', TRUE)
        );
    ...
    public function ajax_update() {
        ...
        $data = array(
            ...
            'JenisPenjualan' => $this->input-> post('txtJenisPenjualan'),
            'Additional' => $this->input-> post('txtAdditional')
